// suite_router — §5.5: shortcuts (F3), tab order, arrow fallback, mouse
// dispatch, popup stamping + compositor clamping (F7, §8.1).
#include <qtty/qtty.h>
#include <QtWidgets>
#include <cstdio>
...
int suite_router() {
    ...
    // compositor clamps an off-screen popup inside the terminal (§8.1)
    ...
    // cursor comes from the focused editor (§5.5)
    edit->setFocus(Qt::OtherFocusReason);
    QCoreApplication::processEvents();
    comp.compose(frame);
