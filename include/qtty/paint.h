// qtty/paint.h — L4 rendering: the custom paint device/engine (§5.4).
...
public:
    ...
    // alignment invariant (§16, verification finding 3).
    int metric(PaintDeviceMetric m) const override;

    CellBuffer &buffer() const { return buf_; }
    QPoint origin;                              // widget-space offset, px
    mutable QVector<CellImage> placements;      // §5.7 funnel output, per frame
...
public:
    // AllFeatures stops QPainter emulating text as paths (§16, F-census).
