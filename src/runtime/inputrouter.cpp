// src/runtime/inputrouter.cpp — L5 (§5.5), rules measured in §16:
...
bool InputRouter::matchShortcut(const KeyEvent &k) {
    ...
    // press: correctness first, the table is small; §5.5).
...
void InputRouter::deliverKey(QWidget *target, const KeyEvent &k) {
    ...
    // Terminals have no key-release; fabricate one immediately (§5.5).
    ...
    // scroll area — the TUI convention (§5.5).
...
void InputRouter::onMouse(const MouseEvent &m) {
    ...
    // Popups first (top of stack down), then the window (§5.5 routing order).
    QWidget *top = win_;
